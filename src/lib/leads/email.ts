...
export async function sendLeadNotification(
  ...
  // Optional UTM row (lp source only)
  const utmRow =
    source === "lp" && (data.utm_source || data.utm_term)
      ? `<tr><td style="padding:4px 12px 4px 0;color:#64748b">Quelle</td><td style="padding:4px 0">${[
          data.utm_source,
          data.utm_medium,
          data.utm_term,
        ]
